...
recorded here as authorised-by-ticket).
Author of record: PM (branch `docs/PM-debt-register`, owner-approved 2026-09-11; this ledger was
authored by PM and is carried by Orchestrator as the single debt ledger). Brought onto `main` by
ticket T7 (#59), AC-1. Nothing in this file changes a product AC. AC text in
`_docs/issues/*.md` is never edited from here — the work orders below say which agent does that,
in which run, under which ticket.

Row owners, one per item, as ticket T7 (#59) AC-1 requires: D5 (`backend/app/main.py`) and D9
(`CONTRIBUTING.md`, `## Constraints Convention`) are owned by Ticket T7; D7 (the B-11 paperwork and
the `_docs/testing.md` test-file list) is owned by Ticket T7; D1 (this `debt-register.md` itself -
its D1 re-judge and the ledger's presence on `main`) is owned by Ticket T7 with PM as author of
record; the D9 *implementation* half (the File Ownership table) stays with Ticket T6.
...
|---|---|---|---|---|
| D1 | `backend/app/database.py` engine now uses `poolclass=NullPool`; `attach_session()` writes the request session into `scope["state"]`. Both exist because AC probes build their own `TestClient` and mount routers themselves | B-10 branch -> main | QA gate B-10 reviews deliberately; hygiene sweep (ticket T7) re-judges once B-17 lands real fixtures | Contract wins over test purity for one store; reverting would make B-10 AC-8/11 unmeasurable; PR description names it "harness-shaped product code". **T7 #59 AC-6 re-judge (performed, T7 SW round 1, at the B-17 branch tip `cdc76fb0be73` = `git log -1 --format=%H origin/issue/B-17-test-isolation`, now an ancestor of `main` via merge `36b80f4`): D1 is kept, not reverted - the `NullPool` engine and `attach_session()` stay in `backend/app/database.py`, because B-17 #58's fixtures (`backend/tests/conftest.py`, autouse `disable_limiter`) replaced no part of what the harness-shaped code exists for: an AC probe still builds its own `TestClient` and mounts routers itself, so it still needs a fresh-engine, no-reconnect pool (`NullPool`) and a session reachable through `scope["state"]` (`attach_session`); the re-judge therefore closes D1 as accepted-permanent, and the D4/D10 import-time-state debt - not `NullPool` - is what B-17's isolation work actually owns** |
...
| D5 | `backend/app/main.py` carries 9 non-ASCII codepoints (U+2011 x8, U+2013) inherited from B-04-era comments | main | Ticket T7 (hygiene sweep) | No AC-14 file set includes `main.py`; byte-identical on both sides of the B-08 merge, so no gate can see it. **CLOSED by T7 #59 AC-4: the 9 codepoints (8 x U+2011, 1 x U+2013, comment/docstring characters on lines 3, 4, 5, 7, 40, 220, 341 of `backend/app/main.py`) are swapped for ASCII hyphens, `backend/app/main.py` now carries 0 non-ASCII codepoints, `ruff` still prints All checks passed!, and AC-4 keeps the invariant by pinning the whole non-ASCII set to zero against the pre-swap snapshot `_docs/baselines/T7-main-pre-swap.py** | Ticket T7 (closed) |
...
| D7 | B-11 grooming defects: DoD/Metadata say 11 ACs where 14 ship; DoD names nonexistent conftest fixtures `env` / `staff_headers`; `testing.md` says 7 test files where 8 exist | `_docs/issues/B-11.md` (merged) | Ticket T7 + docs issue | Found by the QA gate after merge; the product is correct, the paperwork is not. **CLOSED by T7 #59 AC-2 + AC-3 (T7 SW round 1): the two counts in `_docs/issues/B-11.md` now read 14, the 14 AC headings the file ships (Metadata `Owner stub` line, DoD first line), and its Test requirements no longer claims an `env` fixture or a `staff_headers` factory that `backend/tests/conftest.py` does not define; the register's own "7 vs 8" memory of `testing.md` was stale in both directions - AC-3 is now a run-time invariant and `_docs/testing.md`'s `### Structure` block names exactly the 18 `test_*.py` files present in `backend/tests/`** | Ticket T7 (closed) |
...
| D9 | B-08 / B-10 / B-12 Constraints blocks list a file set that ends "Nothing else", and all three branches exceeded it (`services/table_write.py`, `table_conflict.py`, `services/settings.py`, `errors.py`, `schemas.py`, `database.py`, `conftest.py`) while ACs stayed green | three issue files | Ticket T6 (CONTRIBUTING file-ownership + Constraints wording) | ACs and behaviour are the contract; the file lists were written narrower than the ACs needed. **FOURTH CASE OF THIS FAMILY, SELF-INFLICTED, recorded per T7 #59 round 3: T7's own round-2 AC-8 allowed-path regex admitted neither `_docs/issues/B-11.md` (which its AC-2 must edit) nor `_docs/testing.md` (which its AC-3 must edit), so AC-2/AC-3 green forced AC-8 red and AC-8 green forced AC-2/AC-3 red - no implementation state satisfied all three, caught at the groom-to-SW handoff and never by CI. Round 3 widened the regex by exactly those two alternatives and bounded each Constraints exception (B-11: three named line-kinds; testing.md: the `test_*.py` entries inside the `### Structure` block), so Constraints prose and the probe regex are one set again. Convention landed by AC-5 in CONTRIBUTING.md (`## Constraints Convention`), worked example at `_docs/examples/T7-worked-example.md** | Ticket T7 (convention closed), Ticket T6 (File Ownership table) |
| D11 | This ledger (`_docs/debt-register.md`) existed only on branch `docs/PM-debt-register`, so no AC on `main` could read the register it completes (T7 AC-1's dependency discovery) | `docs/PM-debt-register` -> main | Ticket T7 (closed: landed by AC-1 in this issue's diff) | A ledger that lives on an unmerged branch is a private notebook with a longer filename; T7's Constraints list admits this file precisely so the sweep can land it |
...
| Ticket | Scope | Why |
|---|---|---|
| T6 | docs issue: `CONTRIBUTING.md` File Ownership table + Constraints convention (`services/` allowed when ACs force it; how to extend a file list without breaking a frozen AC) - the Constraints-convention half is already landed by T7 AC-5, T6 keeps the File Ownership table half | D9 |
| T7 | docs/hygiene sweep: D5 non-ASCII in `backend/app/main.py` (DONE, AC-4), D7 B-11 paperwork (DONE, AC-2), `testing.md` test-file list (re-scoped from 7-vs-8 to a run-time invariant, DONE at 18-vs-18, AC-3), the D1 re-judge (DONE, AC-6), the D9 Constraints convention in `CONTRIBUTING.md` (DONE, AC-5), and landing this register on `main` (AC-1) | batched to one round; owner for all five items: Ticket T7, author of record PM, gate owner Orchestrator |
