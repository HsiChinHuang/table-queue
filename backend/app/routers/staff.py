"""Staff table endpoints for TableQueue (issue B-08).

Three operations, exactly the ones ``_docs/openapi.yaml`` declares:
...
the one-transaction release) lives in ``app.services.tables`` - no SQL and no ORM query appears
here.

Auth: all three ride on ``app.dependencies.Staff`` (B-04/B-05's ``get_current_staff``), so a
...
B-07 (#33) will add the staff waitlist operations to this same module, so nothing here may assume
the router holds table endpoints only.
"""
...
from app.schemas import TableListResponse, TableResponse, UpdateTableStatusRequest
